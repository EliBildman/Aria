...
const cascade_on = (options) => {
  ...
  return run_sequence((strip) => {
    ...
    for (
      let j = 0;
      j < strip.length && j < options.strip_subset[1] - options.strip_subset[0];
      j++
    ) {
      state.colors.push([]);

      for (let diff = 0; diff < options.strip_subset[0]; diff++)
        state.colors[j].push(null);

      for (
        let k = options.strip_subset[0];
        k < strip.length && k < options.strip_subset[1];
        k++
      ) {
        state.colors[j].push(
          k <= j + options.strip_subset[0] ? options.color : options.baseColor
        );
      }
    }
